 ...
 *  Under MIT License
 *  Last Modification: 2016-03-14 20:16 UTC
 */
...
(function($, window, document, undefined)
{
	...
	function jReportTable(element, options)
	{
		...
		console.log(this.settings, this._defaults, options);
		if ( options.selectors )
			$.extend(this.settings.selectors, this._defaults.selectors, options.selectors);
		if ( options.pagination )
			$.extend(this.settings.pagination, this._defaults.pagination, options.pagination);
		if ( options.render )
			$.extend(this.settings.render, this._defaults.render, options.render);
	...
	jReportTable.prototype = {
		...
		_defaults : {
				types : {
					money : [],
					percent : []
				}, //array column of type money (could be index column or name)
				...
				render : {
					value : function(key, value, row, tr)
					{ //Can change each field data result, and you can take any data from the row
						return value;
					}
				},
				pagination : {
		...
		init : function(context)
		{
			...
			if ( context.settings.custom !== undefined )
				context.pagedata = $.extend({}, context.settings.custom, context.pagedata);

			// Search
			$(context.element).find(context.settings.selectors.search).on('keyup', context, context.rtevents.searchKeyUp);
			...
			if ( $.isArray(context.settings.data) && context.settings.data.length > 0 )
			{
				...
				requestHandler({
					data : context.settings.data,
					records : context.settings.total
				});
			}
			else
			{
		...
		pagelengthset : function()
		{
			...
			{
				$pagelengthset.on('change', this, this.rtevents.pageLengthChange);
				// Cargar set
				if ( $pagelengthset.find('option').length == 0 || ($.isArray(this.settings.pagelengthset) && this.settings.pagelengthset.length > 0) )
				{
					...
					var i;
					this.settings.pagelengthset.sort(function(a, b)
					{
						return a - b
					});
					for (i in this.settings.pagelengthset)
						$pagelengthset.append($('<option value="' + this.settings.pagelengthset[i] + '">' + this.settings.pagelengthset[i]
							+ '</option>'));
				}
				...
				if ( $option.length == 0 )
				{
					$pagelengthset.find('option').each(
						function(iPos, oOption)
						{
							if ( parseInt(oOption.value) > this.settings.pagelength )
							{
								$(oOption).before(
									$('<option value="' + this.settings.pagelength + '" selected="selected">' + this.settings.pagelength
										+ '</option>'));
								return false;
							}
						});
					...
					if ( $pagelengthset.find('option[value="' + this.settings.pagelength + '"]').length == 0 )
						$pagelengthset.append($('<option value="' + this.settings.pagelength + '" selected="selected">' + this.settings.pagelength
							+ '</option>'));
				}
				else
		...
		requestHandler : function(json, textStatus, jqXHR)
		{
			...
			//Header
			var $header = $(this.element).find(this.settings.selectors.columns);
			if ( $header.children().length == 0 )
			{
				var iconSort;
				var toshow = {};
				if ( this.settings.showColumn.length > 0 )
				{
					for (x in this.settings.showColumn)
						toshow[this.settings.showColumn[x]] = this.settings.showColumn[x];
				}
				else
					toshow = json.data[0];
				...
				for ( var x in toshow)
				{
					...
					{
						iconSort = (x !== this.pagedata.sorting ? '' : (this.pagedata.sortorder == 1 ? this.settings.iconOrderClassDesc
							: this.settings.iconOrderClass));
						$header.append("<th>" + x + "<i style='position: absolute;color: " + this.settings.iconColor + ";' class='" + iconSort
							+ "'></i></th>");
					}
				}
				//Sorting
				var ev = this;
				$(this.element).find("th").on('click', this, this.rtevents.clickHeader);
			...
			for (var y = 0; y < json.data.length; y++)
			{
				...
				if ( this.settings.showColumn.length > 0 )
				{
					...
					for (x in this.settings.showColumn)
					{
						x = this.settings.showColumn[x];
						kValue = '';
						if ( typeof json.data[y][x] != 'undefined' )
							kValue = json.data[y][x];
						toshow[x] = kValue;
					}
				}
				else
				...
				for (x in toshow)
				{
					if ( (this.settings.hideColumn.length > 0 && $.inArray(x, this.settings.hideColumn) === -1)
						|| (this.settings.showColumn.length > 0 && $.inArray(x, this.settings.showColumn) !== -1)
						|| (this.settings.hideColumn.length == 0 && this.settings.showColumn.length == 0) )
						$tr.append("<td>" + this.transform(x, toshow[x], json.data[y], $tr) + "</td>");
				}
			...
			$(this.element).find(this.settings.selectors.recordfrom).text(
				1 + this.pagedata.start * this.pagedata.pagelength - this.pagedata.pagelength);
			if ( this.pagedata.total == 0 )
			{
				$(this.element).find(this.settings.selectors.recorduntil).text("1");
				$(this.element).find(this.settings.selectors.rows).append(
					$("<tr/>").append("<td colspan='" + ($(this.element).find("th").length + 1) + "'>" + this.settings.messageNotFound + "</td>"));
			}
		...
		transform : function(key, value, row, tr)
		{
			...
			if ( value === undefined || value == null )
				return '';
			else if ( $.inArray(key, this.settings.types.money) !== -1 )
				return value = "$" + value.toFixed(2);
			else if ( $.inArray(key, this.settings.types.percent) !== -1 )
				return value = value.toFixed(2) + "%";
			else
				$(this.element).find("th").each(function()
				{
					if ( $(this).text() == key && $.inArray($(this).index(), ev.settings.types.money) !== -1 )
						return value = "$" + value.toFixed(2);
				});
		...
		setPages : function()
		{
			var sLinkSelector = this.settings.pagination.container + ' ' + this.settings.pagination.item + '.' + this.settings.pagination.itemClass;
			var sPagesSelector = this.settings.pagination.container + ' ' + this.settings.pagination.item + ':not(.active) > ' + this.settings.pagination.link;
			var sActiveSelector = this.settings.pagination.container + ' ' + this.settings.pagination.item + '[data-page="' + this.pagedata.start + '"]';

			this.redrawPages();
			console.log(sLinkSelector, $(this.element).find(sLinkSelector));

			$(this.element).find(sLinkSelector).removeClass("active");
			$(this.element).find(sActiveSelector).addClass("active");
			$(this.element).find(sPagesSelector).on('click', this, this.rtevents.clickPage);
			$(this.element).find(this.settings.selectors.loading).hide();
		},
		...
		redrawPages : function()
		{
			...
			var $pagination = $(this.element).find(this.settings.pagination.container);
			...
			$previous.addClass(this.settings.pagination.previousClass).append($a.clone().html(this.settings.pagination.previousHTML));
			...
			$next.addClass(this.settings.pagination.nextClass).append($a.clone().html(this.settings.pagination.nextHTML));
			...
			// Create base page
			$page.addClass(this.settings.pagination.itemClass).append($link);
			if ( iStartPage != 1 )
			{
	...
	/*
	 * jQuery Boilerplate - v3.3.2 A jump-start for jQuery plugins development. http://jqueryboilerplate.com
	 *
	 * Made by Zeno Rocha Under MIT License
	 */
